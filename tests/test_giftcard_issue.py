...
from db.dal import DALGiftcards
from db.dal import locked_row_by_id as locked_row_by_id_real
...
from lib.giftcard.agcod.base import (
    AbstractBaseAGCODGiftcardClient,
)
from lib.giftcard.agcod.client import CurrencyCode

if TYPE_CHECKING:
    from lib.giftcard.base import GiftcardPresentable
...
async def _list_events(
    ...
    from db.dal import DAOGiftcardEvents  # late import to avoid cycles
...
async def test_GI6_turns_terminal_inside_locked_section_access_failure(
    ...
    monkeypatch.setattr(
        "lib.giftcard.base.locked_row_by_id", _locked_and_flip, raising=True
    )
